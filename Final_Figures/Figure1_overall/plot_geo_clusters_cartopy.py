...
CANVAS_H = 2*M + 2*G + H_MAIN + 2*H_ROW

# widths
W_ALL = CANVAS_W - 2*M
W3 = (W_ALL - 2*G) / 3       # width of each panel in 3-panel row
W2 = (W_ALL - 1*G) / 2       # width of each panel in 2-panel row
...
RECTS_U = {
    ...
    # middle row (3)
    "p1": (M,             Y_MID, W3, H_ROW),
    "p2": (M + W3 + G,    Y_MID, W3, H_ROW),
    "p3": (M + 2*(W3+G),  Y_MID, W3, H_ROW),

    # bottom row (2)
    "p4": (M,            Y_BOT, W2, H_ROW),
    "p5": (M + W2 + G,   Y_BOT, W2, H_ROW),
}
...
def plot_year(
    ...
    fig_h = 7.0 * (CANVAS_H / 100.0)
    fig = plt.figure(figsize=(8.4, fig_h), facecolor="white")
